...
class EcommerceSpider(object):
    ...
    def extract_item(self, html, link, ID):
        ...
        EspList = dict(Nome=Nome, EAN=ID)

        try:
            imgs = html.xpath("img[@class='zoom_small_thumb']/@src")
            imgcont = 1
            for img in imgs:
                EspList["Imagem "+str(imgcont)] = img
                imgcont += 1
        except IndexError as e:
            EspList["Imagem"] = "Not Found"

        desc = html.xpath("//div[@id='tab-description']//p//text()")
        attcont = 1
        for x in desc:
...
spider.crawl_to_file(filee+"-ObraFacil.csv")
