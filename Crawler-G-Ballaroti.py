...
class EcommerceSpider(object):
    ...
    def extract_item(self, html, link, ID):
        EspList = {}
        try:
            Nome = html.xpath("//div[@class='title']/h1/div")[0].text
        ...
        imgcont = 1
        for img in imgs:
            EspList["Imagem "+str(imgcont)] = img.replace("80-80", "1000-1000")
            imgcont += 1

        attr = html.xpath("//div[@id='caracteristicas']//table//tr")

        for att in attr:
            EspList[formatar(att[0].text)] = att[1].text
...
spider.crawl_to_file("Padovani-Madeira.csv")
