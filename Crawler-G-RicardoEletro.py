...
class EcommerceSpider(object):
    ...
    def extract_item(self, html, link, ID):
        ...
        try:
            nome = html.xpath(
                "//div[@id='ProdutoDetalhesNomeProduto']/h1")[0].text
        except IndexError as e:
            nome = "Nao Encontrado"
        try:
            desc = html.xpath("//div[@id='aba-descricao']/div")[0].text
        except IndexError as e:
            desc = "Não Encontrado"
        ...
        img = html.xpath(
            "//div[@class='produto-detalhes-fotos-pequenas']/a/img/@src")

        imgcont = 1

        for x in img:
            EspList["Imagem "+str(imgcont)] = x.replace("87x87", "500x500")
            imgcont += 1

        atrib = html.xpath("//div[@id='aba-caracteristicas']/div/table//tr")
        cont_atrib = 1

        for elem in atrib:
            if(len(elem) == 1):
                EspList["Atributo "+str(cont_atrib)] = elem[0].text
                cont_atrib += 1
            elif(len(elem) == 2):
                EspList[formatar(elem[0].text)] = elem[1].text
            else:
                print("MOIO")
