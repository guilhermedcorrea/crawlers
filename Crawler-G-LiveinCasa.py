...
class EcommerceSpider(object):
    ...
    def extract_item(self, html, link, ID):
        EspList = {}
        try:
            Nome = html.xpath("//div[@class='hide-mobile']/h1")[0].text
        except IndexError as e:
            Nome = "Not Found"

        try:
            Desc = html.xpath("//div[@id='tab-description']//text()")
            totaldesc = ""
        ...
        try:
            mainimg = html.xpath(
                "//div[@class='thumbnails primary-thumb']/img/@src")[0]
        except IndexError as e:
            mainimg = "Not Found"

        EspList = dict(EAN=ID, Nome=Nome, Descrição=blank(
            totaldesc), MAINIMG=mainimg)

        try:
            att = html.xpath("//div[@id='tab-especificacoes']//tr")
            for atributo in att:
                EspList[atributo[0].text] = atributo[1].text
        except IndexError as e:
            att = "Not Found"

        imgs = html.xpath("//a[@class='thumbnail']/@href")
        imgcont = 1
...
spider.crawl_to_file(filee+"-Live.csv")
