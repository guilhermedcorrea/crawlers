...
class EcommerceSpider(object):
    def __init__(self, start_url):
        ...
        self.end_time = None
        self.cats = ['http://www.startecimport.com.br/produtos/utilizacao/todos', 'http://www.startecimport.com.br/produtos/led/todos', 'http://www.startecimport.com.br/produtos/kids-personagens/todos',
                     'http://www.startecimport.com.br/produtos/area-externa/todos', 'http://www.startecimport.com.br/produtos/escritorio/todos', 'http://www.startecimport.com.br/produtos/multiuso/todos', 'http://www.startecimport.com.br/produtos/quarto-sala/todos']
        self.file = "IPC.csv"
    ...
    def get_items(self):
        for SKU in self.CodRef:
            r = requests.get(self.links[SKU], headers)
            html = parser.fromstring(r.content)
            print(str(r) + " : " + self.links[SKU])
            self.items.append(self.extract_item(html, self.links[SKU], SKU))
    ...
    def extract_item(self, html, link, SKU):
        EspList = {}
        try:
            nome = html.xpath(
        ...
        try:
            desc = html.xpath(
                "//div[@class='woocommerce-product-details__short-description']/p[1]")[0].text
        except IndexError as e:
            desc = "Not Found"

        EspList = dict(Nome=nome, SKU=SKU, Descrição=desc)

        carac = html.xpath("//div[@class='et_pb_text_inner']/ul/li")
        caracont = 1

        for x in carac:
            EspList["Atributo "+str(caracont)] = x.text
            caracont += 1

        attr = html.xpath("//div[@class='et_pb_tab_content']/table[1]//tr")
        for x in attr:
            if(x[0].text == " "):
                print(x[0].text)
            if(x[0].text != None):
                EspList[formatar(x[0].text)] = x[1].text.replace("\xa0", "")

        imgs = html.xpath(
            "//figure[@class='woocommerce-product-gallery__wrapper']/div/a/@href")
        imgcont = 1
        ...
        links = html.xpath(
            "//div[@class='woocommerce-product-details__short-description']//a/@href")
        linkcont = 1

        for x in links:
            EspList["PDF "+str(linkcont)] = x
            linkcont += 1
