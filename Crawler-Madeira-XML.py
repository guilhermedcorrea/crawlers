...
class EcommerceSpider(object):
    def __init__(self, start_url):
        ...
        self.set_base_url()
        self.key = []
        self.remove = ["Categoria_Google","EAN_GOOGLE","Image_Google","MPN_Google","SKU_Google","Descrição 2", "Categoria", "Descrição2","SKU", "cat", "DescInfo", "Foto", "VIXI"]
        self.removed = []
        self.errors = 0
    ...
    def extract_item(self, html, link):
        ...
        try:
            ...
            try:
                EAN = re.search("\d\d\d\d\d\d\d\d\d\d\d\d\d",
                                Json_madeira["image"])
                if EAN == None:
                    EAN = "Not Found"
                else:
    ...
    def save_items(self, filename):
        ...
        send_email(filename+" finalizado")
        ftp_send(filename, "/Controle")
...
linka = LinkCreator()
linka.CSVREADER()
for sku in linka.sku:
    print(linka.cats[sku])
    spider = EcommerceSpider(linka.cats[sku])
    FILENAME = sku + B_FILENAME
